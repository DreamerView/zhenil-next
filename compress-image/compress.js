...
fs.readdir(testFolder, (err, files) => {
    files.forEach(file => {
        sharp(path.resolve(__dirname,'../public/services/convert/'+file)).toFormat('webp').webp({ quality: 70 }).toFile(path.resolve(__dirname,'../public/services/'+path.parse(file).name+'.webp'));
    });
});
